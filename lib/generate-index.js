...
function htmlToMarkdown (html) {
  ...
  return html
    ...
    .replace(/<table>(.*?)<\/table>/gs, function (match, tableContent) {
      ...
      if (headerMatch) {
        const headerCells = headerMatch[1].match(/<th>(.*?)<\/th>/gs) || []
        if (headerCells.length > 0) {
          markdown += '| ' + headerCells.map((cell) => cell.replace(/<th>(.*?)<\/th>/s, '$1')).join(' | ') + ' |\n'
          markdown += '| ' + headerCells.map(() => '---').join(' | ') + ' |\n'
        }
      }

      // Extract body rows
      const bodyMatch = tableContent.match(/<tbody>(.*?)<\/tbody>/s)
      if (bodyMatch) {
        const rows = bodyMatch[1].match(/<tr>(.*?)<\/tr>/gs) || []
        rows.forEach((row) => {
          const cells = row.match(/<td>(.*?)<\/td>/gs) || []
          if (cells.length > 0) {
            markdown += '| ' + cells.map((cell) => cell.replace(/<td>(.*?)<\/td>/s, '$1')).join(' | ') + ' |\n'
          }
        })
      }
