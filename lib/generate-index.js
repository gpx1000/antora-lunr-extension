...
function generateIndex (playbook, contentCatalog, { indexLatestOnly = false, languages = ['en'], logger } = {}) {
  ...
  documents.forEach((doc) => {
    doc.titles.forEach((title) => {
      if (setTitles.has(title)) {
        const idsUnique = new Set()
        idsUnique.add(doc.id)
        const words = (title.text).toLowerCase().split(/\W+/).filter((w) => w)
        words.forEach((word) => store.trie.insertWithData(word, idsUnique))
        setTitles.delete(title)
      }
    })
  })
...
function createIndexFile (index) {
  const trieData = index.store.trie.save()
  return {
    mediaType: 'application/javascript',
    contents: Buffer.from(`antoraSearch.initSearch(lunr, ${JSON.stringify(index)}, ${trieData})`),
