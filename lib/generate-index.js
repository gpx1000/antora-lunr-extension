...
function createMarkdownIndexFile (index) {
  ...
  return documents
    ...
    .map((doc) => {
      ...
      // Create markdown content with just the text from the document
      const markdownContent = doc.text

      return {
        mediaType: 'text/markdown',
        contents: Buffer.from(markdownContent),
        src: { stem: filename },
        out: { path: path.join(dirName, `${filename}.md`) },
        pub: { url: `/${dirName}/${filename}.md`, rootPath: '' },
      }
    })
}
