...
function register ({ config: { languages, indexLatestOnly, snippetLength = 100, ...unknownOptions } }) {
  ...
  this.on('beforePublish', ({ playbook, siteCatalog, contentCatalog }) => {
    ...
    // Add markdown index file for LLM support
    logger.info('Generating markdown index file for LLM support')
    siteCatalog.addFile(generateIndex.createMarkdownIndexFile(index))
  })
}
